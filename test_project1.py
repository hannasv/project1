...
from functions import generateDesignmatrix, franke_function
...
def test_design(p=1):
    if (p==1):
        ...
        #    new[:, i] = X[:i]
        assert np.all(abs(X- generateDesignmatrix(p,x,y))<1e8)
        #assert np.allclose(X, generateDesignmatrix(p,x,y))
        ...
        X = [1, x, y, x**2, x*y, y**2, x**3, x**2*y, x*y**2, y**3, x**4, x**3*y, x**2*y**2, x*y**3, y**4]
        assert np.all(abs(X- generateDesignmatrix(p,x,y))<1e8)
...
def test_train_test_split():
    test that the indice subset are disjoint --> empty section. 
